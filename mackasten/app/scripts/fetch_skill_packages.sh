#!/usr/bin/env bash
# Fetch mackasten-package manifests + shortcut XML from the public skill repo
# into Vendor/SkillPackages/ for the Mackasten iOS app.
#
# Dual layout: prefers mackasten/packages + mackasten-package.v1.json;
# falls back to legacy horizon/packages + horizon-package.v1.json.
#
# From skill monorepo (dev):
#   ./mackasten/app/scripts/fetch_skill_packages.sh
#
# From private app repo (script at ./scripts/):
...
# Detect layout:
#  A) skill repo:  <skill>/mackasten/app/scripts/this.sh  + <skill>/mackasten/packages
...
if [[ -d "$SCRIPT_DIR/../../packages" && -f "$SCRIPT_DIR/../../packages/hello-world/package.json" ]]; then
  ...
  OUT_DIR="${OUT_DIR:-$SKILL_ROOT/mackasten/app/Vendor/SkillPackages}"
elif [[ -d "$SCRIPT_DIR/../mackasten/packages" || -d "$SCRIPT_DIR/../horizon/packages" ]]; then
  SKILL_ROOT="$(cd "$SCRIPT_DIR/.." && pwd)"
...
# Prefer Mackasten layout; fall back to legacy Horizon paths.
if [[ -d "$SKILL_ROOT/mackasten/packages" ]]; then
  PACKAGES_SRC="$SKILL_ROOT/mackasten/packages"
  LAYOUT=mackasten
elif [[ -d "$SKILL_ROOT/horizon/packages" ]]; then
  PACKAGES_SRC="$SKILL_ROOT/horizon/packages"
  LAYOUT=horizon
else
  echo "FAIL no mackasten/packages or horizon/packages in $SKILL_ROOT" >&2
  exit 1
fi

if [[ -f "$SKILL_ROOT/data/schemas/mackasten-package.v1.json" ]]; then
  SCHEMA_SRC="$SKILL_ROOT/data/schemas/mackasten-package.v1.json"
  SCHEMA_OUT_NAME="mackasten-package.v1.json"
elif [[ -f "$SKILL_ROOT/data/schemas/horizon-package.v1.json" ]]; then
  SCHEMA_SRC="$SKILL_ROOT/data/schemas/horizon-package.v1.json"
  SCHEMA_OUT_NAME="horizon-package.v1.json"
else
  echo "FAIL missing mackasten/horizon package schema in $SKILL_ROOT" >&2
  exit 1
fi

echo "Using layout=$LAYOUT packages=$PACKAGES_SRC schema=$(basename "$SCHEMA_SRC")"

rm -rf "$OUT_DIR"
mkdir -p "$OUT_DIR/schema"
cp "$SCHEMA_SRC" "$OUT_DIR/schema/$SCHEMA_OUT_NAME"

python3 - "$PACKAGES_SRC" "$SKILL_ROOT" "$OUT_DIR" "$SKILL_REF" "$LAYOUT" <<'PY'
...
for man in sorted(packages_src.glob("*/package.json")):
    ...
    data["_mackasten_fetch"] = {"skill_ref": skill_ref, "layout": layout}
...
SCHEMA_FOR_VALIDATE="$OUT_DIR/schema/$SCHEMA_OUT_NAME"
if [[ -f "$SKILL_ROOT/scripts/check_json_schema.py" ]]; then
  echo "Validating package.json against schema…"
  python3 - "$OUT_DIR" "$SCHEMA_FOR_VALIDATE" "$SKILL_ROOT" <<'PY'
import sys
from pathlib import Path
