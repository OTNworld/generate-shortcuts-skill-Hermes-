# URL schemes — Mackasten + Shortcuts
...
- Allowlist prefixes MVP: `templates/`, `mackasten/`.
...
Use when Mackasten needs success/failure return:

```text
shortcuts://x-callback-url/run-shortcut?name=…&x-success=…&x-error=…
```

Success URL should open back into Mackasten (`hermes-shortcuts://` or `mackasten://` if added).
