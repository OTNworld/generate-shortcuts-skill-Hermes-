# Horizon — vision produit finale

**Status:** target architecture (2026-07-27). Runtime app = repo privé `horizon-iOS`.  
**Seed / skill agent :** ce dossier. **Paper packages :** [`../packages/`](../packages/).

## Thesis

Horizon est l’**app compagnon** du skill Hermes Shortcuts : là où le skill **écrit /
valide / atteste** des raccourcis, Horizon **découvre, installe, exécute et
...
|--------|--------------|------|
| Skill Hermes (public MIT) | `generate-shortcuts-skill-Hermes-` | Grammaire, goldens, validate, sign, attest, manifests `horizon-package/v1` |
| App Horizon (privé) | `horizon-iOS` | UX, confiance, updates, routing modèles, Siri |
...
2. **Consommateur** — ouvre Horizon, parcourt la marketplace, installe, lance par UI ou Siri.
...
- Run via `shortcuts://run-shortcut` ou App Intent Horizon qui délègue.
- Pas de réimplémentation du moteur Shortcuts — Horizon orchestre.
...
Un utilisateur peut : ouvrir Horizon → voir les 4 packages seed → installer Hello World →
