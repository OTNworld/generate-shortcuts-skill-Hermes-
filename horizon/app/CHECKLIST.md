# Horizon iOS — creation & test checklist
...
- [ ] Private repo `OTNworld/horizon-iOS` exists
...
- [ ] Reject malformed schema (`schema` ≠ `horizon-package/v1`)
...
```bash
cd /path/to/horizon-iOS
./scripts/fetch_skill_packages.sh
xcodegen generate
xcodebuild -scheme Horizon -destination 'platform=iOS Simulator,name=iPhone 17' test
```
