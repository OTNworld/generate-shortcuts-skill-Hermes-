...
name: horizon-ios
description: >
  Use when building, scaffolding, or shipping the Horizon companion iOS app:
  ...
  horizon-package/v1 manifests from the generate-shortcuts-skill-Hermes- skill repo.
  Prefer this skill over generic iOS scaffolding when the product is Horizon.
version: 0.1.0
...
metadata:
  hermes:
    tags: [horizon, ios, swiftui, app-intents, shortcuts, marketplace, siri]
    related_skills: [shortcuts-generator, ios-dev, app-intents]
    app_repo: OTNworld/horizon-iOS
    skill_repo: OTNworld/generate-shortcuts-skill-Hermes-
---

# Horizon iOS — companion app skill

Protocole agent pour **oneshot** (ou itérer) l’app native **Horizon** : marketplace
de packages Shortcuts, runtime local-model, Siri / App Intents, deep links vers
l’espace skill.

Ce skill **ne remplace pas** un skill iOS générique : il ancre le produit Horizon.
...
- Créer ou étendre l’app `horizon-iOS`.
- Brancher le fetch CI des `horizon/packages/*/package.json` depuis le skill public.
...
| Repo privé `OTNworld/horizon-iOS` | Code app (ce dossier est le seed) |
...
5. **Catalog** : parser `horizon-package/v1` ; afficher les 4 SKUs seed.
...
- **Ne pas** inventer d’App Intent Apple IDs — seulement intents **Horizon** ou exports vérifiés.
...
- `references/APP_INTENTS.md` — intents Horizon + Siri
- `references/TESTING.md` — pyramides Linux / Mac / device
- `references/DESIGN.md` — direction visuelle Horizon
- Skill packages SSOT : `../packages/` + `../../data/schemas/horizon-package.v1.json`
